...
//		cout<<" Takes as argument this thang: "<< lfArgumentPointerNameList.at(i)<<endl;
	...
	// We only need to add this dependency for the DORreactantIndex because the
	// local functions are exclusively evaluated on molecules mapped to this reactant.
	...
	) : ReactionClass(name,baseRate,baseRateName,transformationSet,s)
{
	...
	// Add type I molecule dependencies, so that when this function
	// is reevaluated on a molecule, the molecule knows to update this reaction.
	// We only need to add these dependencies for the DOR reactants because the
	// local functions are exclusively evaluated on molecules mapped to these reactants.
