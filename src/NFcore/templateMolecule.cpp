...
bool TemplateMolecule::checkSymmetry(TemplateMolecule *tm1, TemplateMolecule *tm2, string bSite1, string bSite2)
{
	...
	std::vector<bool> mappedConstraint(tm2->n_compStateConstraint, false);
		...
		for(int j=0; j<tm2->n_compStateConstraint; j++) {
			if(tm1->compStateConstraint_Comp[i] == tm2->compStateConstraint_Comp[j])
				if(tm1->compStateConstraint_Constraint[i] == tm2->compStateConstraint_Constraint[j])
					if(!mappedConstraint[j]) {
						mappedConstraint[j]=true;
						break;
					}
		}
	}
	for(int j=0; j<tm2->n_compStateConstraint; j++) {
		if(!mappedConstraint[j]) return false;
	}
	...
	for(int j=0; j<tm2->n_compStateExclusion; j++) {
		if(!mappedExclusion[j]) return false;
	}
	...
	for(int j=0; j<tm2->n_occupiedComps; j++) {
		if(!mappedOccupied[j]) return false;
	}
	...
	for(int j=0; j<tm2->n_connectedTo; j++) {
		if(!mappedConnected[j]) return false;
	}
	...
	// Note: The heuristic structural checks above evaluate localized patterns and shallow binding
	// structures rather than performing a rigorous full-graph isomorphism analysis of recursively
	// connected template components. While theoretically incomplete for complex multi-molecule
	// symmetry detection, this localized matching is sufficient for practical simulation rule
	// resolution and avoids the performance penalty and complexity of deep recursive checks.
