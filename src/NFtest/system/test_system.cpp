...
void NFtest_system::run()
{
	...
	// Test getMoleculeTypeByName with non-existent name
	bool threw = false;
	try {
	...
	if (!threw) {
		throw std::runtime_error("System::getMoleculeTypeByName did not throw std::runtime_error for non-existent MoleculeType.");
	}
