		...
		bool verbose)
{
	...
	try {
		...
		if (mgids.size()>0){
			...
			unsigned int mgids_size = mgids.size();
			for(unsigned int img=0; img<mgids_size; img++) {
				molec_vec[mgids[img]] = mids[img];
			}
			// AS2023 - now we use it to compress the initial state vector
			int last_val = molec_vec[0];
			int val_ctr = 1;
			unsigned int molec_vec_size = molec_vec.size();
			for(unsigned int ici=1; ici<molec_vec_size; ici++) {
		...
		for(int k=0;k<operations_size;k++) {
			logstr += "        " + operations[k] + ",\n";
		}
		// AS2023
